...
function encodeToRot13(str) {
    ...
    var alphUp = "abcdefghijklmnopqrstuvwxyz".toUpperCase().split("");
    var arrMess = str.split("");
    let newMess = "";
    
    function isLetter(l) {
      for (var p = 0; p < alphabet.length; p++) {
        if (alphabet[p] === l || alphUp[p] === l) return true;
      }
      return false;
    }
    
    for (var i = 0; i < arrMess.length; i++) {
      if (isLetter(arrMess[i])) {
        if (arrMess[i] === arrMess[i].toUpperCase()) {
          for (var k = 0; k < alphUp.length; k++) {
            if (arrMess[i] === alphUp[k]) {
              if (k > 12) newMess += alphUp[k-13];
              else newMess += alphUp[k+13];
            }
          }
        } else if (arrMess[i] === arrMess[i].toLowerCase()) {
          for (var j = 0; j < alphabet.length; j++) {
            if (arrMess[i] === alphabet[j]) {
              if (j > 12) newMess += alphabet[j-13];
              else newMess += alphabet[j+13];
            }
          }
        }
      } else {
