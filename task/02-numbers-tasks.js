...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    ...
    let lengthComposition = Math.sqrt(Math.pow(x1, 2) + Math.pow(y1, 2)) * Math.sqrt(Math.pow(x2, 2) + Math.pow(y2, 2));
    let result = Math.acos(dotProduct / lengthComposition);
    return result;
}
...
function roundToPowerOfTen(num, pow) {
    if (pow === 0) return Math.round(num / 1) * 1;
